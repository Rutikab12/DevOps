...
(function($) {
    $.fn.flowchat = function(options) {
        var settings = $.extend({ delay: 1500, startButtonId: '#startButton', autoStart: !0, startMessageId: 1, dataJSON: null }, options);
        var container = $(this);
        $(function() {
            if (settings.autoStart)
                startChat(container, settings.dataJSON, settings.startMessageId, settings.delay)
        });
        $(document).on('click', settings.startButtonId, function() { startChat(container, settings.dataJSON, settings.startMessageId, settings.delay) })
    }
    ...
    function addOptions(container, data, delay, m) {
        ...
        for (var i = 1; i < 6; i++) {
            ...
            if (optionText != "" && optionText != undefined && optionText != null) {
                ...
                $optionElem.click(function() { selectOption($(this), container, data, delay) });
